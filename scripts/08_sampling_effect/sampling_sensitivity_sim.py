...
Typography : Arial (falls back to Liberation Serif if absent)
...
# Analytic NB slope + median voltage + sigmoid fit are now in vgsot-sim.
from vgsot_sim.analysis import nb_fit
from vgsot_sim.analysis.variability import fit_sigmoid_to_average

# ── Physical baseline (same as variability_sim.py) ─────────────────────────
TAU0, DELTA0, VC0, TW = 1.0, 4.91, 0.857, 0.75
VTH_MEAS, BETA_MEAS = 0.894, 44.6
BETA_NB_ANALYTIC = nb_fit.beta_nb_analytic(DELTA0, VC0)
...
VTH_NB = float(nb_fit.vth_nb(TW, DELTA0, VC0, tau0=TAU0))

def fit_sigmoid(P_mean):
    """Thin alias retaining the original (V_th, beta) return signature."""
    return fit_sigmoid_to_average(V_DENSE, P_mean,
                                   p0=(VTH_NB, BETA_NB_ANALYTIC),
                                   bounds=([0.0, 0.1], [1.5, 100]))
...
# Use a distinct base seed per CV (with N_REF-sized resamples) so each
# F_ref is a genuinely independent reference, not three views of the same
# underlying draws with only the parameter shifted. Sharing seed=1234
# across CVs causes the inter-CV F-spread to be artificially correlated.
REF_BASE_SEED = 1234
N_REF_REPLICAS = 5      # cheap variance estimate of the reference itself

F_REF = {}
F_REF_STD = {}
for cv in CVS:
    base = REF_BASE_SEED + int(round(cv * 10000))
    replicas = []
    for r in range(N_REF_REPLICAS):
        rng = np.random.default_rng(seed=base + r * 7919)   # 7919 prime
        beta_big = mc_beta_eff(N_REF, cv, rng)
        replicas.append(beta_big / BETA_NB_FIT)
    F_REF[cv] = float(np.mean(replicas))
    F_REF_STD[cv] = float(np.std(replicas, ddof=1))
    print(f"F_ref(CV = {cv:.3f}, N = {N_REF:,}) = {F_REF[cv]:.5f} "
          f"± {F_REF_STD[cv]:.5f}   "
          f"(mean of {N_REF_REPLICAS} independent replicas)")
print()
...
ax_a.legend(loc="lower right", title=r"$\mathrm{CV}_\Delta$")
ax_a.xaxis.set_major_formatter(mticker.ScalarFormatter())

# annotate reference lines
ax_a.text(11500, F_REF[0.077], r" $F_{\rm ref}$", va="center", color=AMBER)
...
ax_d.set_xticklabels([LABELS[cv] for cv in CVS])
ax_d.set_ylabel(r"Minimum $N$  (95% confidence)")
ax_d.set_title(
    r"Smallest $N$ meeting relative-error tolerance"
)
ax_d.set_ylim(10, 60000)
ax_d.legend(loc="upper left", title=r"tolerance on $\hat F$")
...
outpath = OUTDIR + "Chapter02_local_18.png"
