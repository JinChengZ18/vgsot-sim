...
为检验上述$$\Delta$$单参数投影是否遗漏端电压驱动下的器件级展宽，本工作进一步将同一PDK失配预算注入宏自旋求解器：由$$R_P$$残余失配采样$$D$$与$$D_{\mathrm{elec}}$$，由$$t_f$$、$$M_s$$与TMR代理采样自由层厚度、饱和磁化和各向异性参数，并将$$R_{\mathrm{SOT}}$$失配映射到$$R_W$$。该设置在固定$$|V_{\mathrm{SOT}}|$$下尤其关键，因为$$R_W$$变化会同步改变每个样本实际获得的$$|I_{\mathrm{SOT}}|$$与SOT自热功率。图2.17显示标称器件在实测阈值附近保留清晰的Sigmoid上升段，而同一PDK预算下的晶圆平均曲线明显趋缓：$$P_{\mathrm{sw}}$$自$$0.25$$升至$$0.5$$所需的电压跨度由$$51\,\mathrm{mV}$$增至$$198\,\mathrm{mV}$$，过渡区展宽约$$3.9$$倍，同时在标称阈值以下出现翻转尾部[^note-e7-macrospin]。两种投影口径的差距由此界定：图2.16的$$\Delta$$单参数传递函数在PDK基线处给出$$\mathcal{F}=0.997$$，即晶圆平均斜率相对单器件几乎不变，而固定端电压下的完整失配注入给出数倍展宽。该差距来自$$\Delta$$投影中没有对应量的两条通道，即样本间阈值横向散布与$$R_{\mathrm{SOT}}$$引起的端电压-驱动电流换算漂移。故表2.11的工艺裕度应理解为$$\Delta$$单参数投影口径下的结果，阵列级Sigmoid斜率的预算须在器件级求解器中按端口条件注入几何、磁性与沟道电阻的联合失配。
[^note-e7-macrospin]: 统计规模为16个D2D失配器件×每点32次热噪声试验 (池化512次)，标称曲线的Wilson 95%半宽自早期6×8设置的$$\pm13\%$$收紧至$$\pm4.3\%$$，两条曲线的差异因此超出统计带宽；晶圆平均曲线是16个器件级估计的平均，其不确定度以器件间散布为主，图中阴影仅给出二项分量。展宽以无拟合的电压跨度度量，原因是晶圆平均曲线在插图窗口内未达饱和 (最高$$0.633$$)，四参数logistic的基线与幅度近简并、斜率随基线约束在$$5$$–$$9\,\mathrm{V^{-1}}$$间漂移；以物理约束$$y_0\ge0$$拟合时标称$$\beta_s=30.3\pm5.0\,\mathrm{V^{-1}}$$、晶圆平均$$9.1\pm4.9\,\mathrm{V^{-1}}$$ ($$R^2$$分别为$$0.996$$与$$0.988$$)，与跨度度量给出的展宽倍数同向。标称曲线原始50%穿越点$$896\pm7\,\mathrm{mV}$$与2.3.2节实测拟合锚点$$894\,\mathrm{mV}$$一致，其logistic斜率$$30.3\,\mathrm{V^{-1}}$$亦与2.3.4节加密扫描的$$\beta_s^{\mathrm{sLLG}}=33.8\pm13.5\,\mathrm{V^{-1}}$$相符，两项构成本节失配注入与主基准之间的自洽核验。晶圆平均曲线穿越点上移至$$965\pm14\,\mathrm{mV}$$，源于高阈值样本推迟饱和；低阈值样本在$$0.62$$–$$0.78\,\mathrm{V}$$贡献$$0.04$$–$$0.26$$的翻转尾部则抬高低压端，两端合并使中段趋缓。

---

![宏自旋工艺失配Monte Carlo下的Psw端电压扫描](figs/Chapter02_local_17.png)

**图2.17** 工艺失配下的端电压写入概率Monte Carlo曲线，$$t_p = 0.75\,\mathrm{ns}$$、$$V_{\mathrm{MTJ}} = 0$$。主图将图2.11的$$|I_{\mathrm{SOT}}| = 300\text{-}3500\,\mu\mathrm{A}$$宽谱扫描按标称$$R_W$$映射为固定$$|V_{\mathrm{SOT}}|$$轴，插图放大$$800\text{-}1400\,\mu\mathrm{A}$$阈值窗口对应的电压区间。蓝色圆线为标称宏自旋曲线，红色方线为PDK失配样本的晶圆平均，阴影为按池化试验数给出的Wilson 95%区间 (对失配平均曲线仅含二项分量)，绿色虚线为同批次实测$$V_{\mathrm{th}} = 894\,\mathrm{mV}$$参考线；工艺样本采用16个D2D失配器件、每点32次热噪声试验 (池化512次，保模长Cayley积分器、自热开启)。插图中标称曲线在阈值附近仍呈Sigmoid上升，失配平均曲线则在标称阈值以下即出现翻转尾部、在阈值以上推迟饱和，两端合并使中段变浅；这反映不同器件阈值横向散布以及$$R_{\mathrm{SOT}}$$引起的端电压到驱动电流换算漂移，而非单器件Sigmoid特征消失。
