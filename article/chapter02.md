...
[^note-dev-hex]: 该垂直关系在标定中被确认为模型的一处方向敏感点，实现中将$$\mathbf{H}_{\mathrm{EX}}$$约束为垂直于$$\hat{\sigma}_{\mathrm{SH}}$$ (本仿真器默认$$-\hat{x}$$)、默认取沿$$-\hat{y}$$的$$-50\,\mathrm{Oe}$$。幅值与角度的系统扫描 (0.75 ns工作点，幅值档11点×150条、角度档每点200条轨迹) 给出定量边界。偏轴退化为渐进型：$$50\,\mathrm{Oe}$$下$$1400\,\mu\mathrm{A}$$过驱点的$$P_{\mathrm{sw}}$$随偏轴角$$0/10/20/30/45°$$为$$0.73/0.59/0.58/0.46/0.41$$，约$$10°$$内平台降约$$0.13$$，超过约$$30°$$确定性偏置丧失、过驱$$P_{\mathrm{sw}}$$跌至$$0.5$$以下。幅值窗口同样有界：$$H=0$$下翻转仍有阈值，但越过阈值后$$P_{\mathrm{sw}}$$饱和于$$\approx0.5$$而非趋于1，器件退化为不受电流极性偏置的无偏硬币 (与正文对称性破缺机制互为印证)；$$25\,\mathrm{Oe}$$峰值最高 ($$0.87$$，扫描端仍在上升)，$$50\,\mathrm{Oe}$$给出平坦平台 ($$0.71$$–$$0.74$$)；$$100\,\mathrm{Oe}$$起退化，$$150$$–$$200\,\mathrm{Oe}$$过驱区$$P_{\mathrm{sw}}$$跌破$$0.5$$，$$300\,\mathrm{Oe}$$退化为$$0.5$$附近的非单调涨落；以首次越过$$0.5$$的名义交叉计，$$200\,\mathrm{Oe}$$较$$50\,\mathrm{Oe}$$上移约$$12\%$$ (该场强下已无稳定高于$$0.5$$的平台，交叉仅为名义值)。后者即2.3节实验条件 ($$H_x=200\,\mathrm{Oe}$$) 与实现默认值之间口径差的来源：有效$$\theta_{\mathrm{SH}}$$标定吸收了幅值差额，但模型在$$200\,\mathrm{Oe}$$的高场行为与实测的干净饱和不符，推测源于未建模的界面DMI与粗糙度通道，故行为级标定取$$50\,\mathrm{Oe}$$等效场为宜。
...
上述温度机制的接口级后果可由求解器直接量化：固定0.75 ns写入口径、自热开启，将环境温度自250 K扫至400 K，50%阈值电压以$$-2.18\,\mathrm{mV/K}$$近似线性漂移，300 K处899 mV与实测锚点894 mV在单点MC统计分辨率内一致；对固定偏压的概率工作点，这对应中点附近$$\mathrm{d}P_{\mathrm{sw}}/\mathrm{d}T\approx0.015$$–$$0.024\,\mathrm{K^{-1}}$$的灵敏度 (logistic中点斜率为$$\beta_s/4$$)，环境温度漂移10 K即造成约一个Sigmoid尺度参数$$k$$的阈值移动，后续章节的概率接口须以偏压随温度回归或差分参考消除该漂移[^note-e6-temp]。
[^note-e6-temp]: 扫描设置沿用图2.11口径 (Cayley积分器、自热开启、AP→P、0.75 ns写入+3.25 ns弛豫)，每温度11点×200条轨迹。六档$$T_0=250/275/300/325/350/400\,\mathrm{K}$$的50%阈值为$$1007/937/899/832/780/678\,\mathrm{mV}$$ (单点MC分辨率6–11 mV)，线性拟合斜率$$-2.18\,\mathrm{mV/K}$$。过渡区斜率$$\beta_s$$在250–350 K间处于26–44 V$$^{-1}$$，序列非单调、表观上升主要由350 K单点 ($$43.7\pm13.9$$，95%区间) 驱动，误差加权趋势$$0.05\pm0.05\,\mathrm{V^{-1}K^{-1}}$$不显著；400 K下电压窗口在50%穿越点以下仅余一个采样点，四参数拟合退化，斜率在本采样下不可分辨 (数据与$$\beta_s\approx42\,\mathrm{V^{-1}}$$、即与350 K相当的斜率同样相容)。$$\mathrm{d}P_{\mathrm{sw}}/\mathrm{d}T$$的区间端点取$$\beta_s/4\cdot|\mathrm{d}V_{\mathrm{th}}/\mathrm{d}T|$$，$$\beta_s$$分别为仿真口径 (各温度点方差加权均值约$$28\,\mathrm{V^{-1}}$$) 与实测口径 ($$44.6\,\mathrm{V^{-1}}$$)。
...
直接代入极限值可验证该表达式的物理自洽性：$$m_z=+1$$给出$$R_{\mathrm{MTJ}}=R_P$$ (平行态，约$$5\,\mathrm{k\Omega}$$)，$$m_z=-1$$给出$$R_{\mathrm{MTJ}}=R_P(1+\mathrm{TMR})=R_{AP}$$ (反平行态，约$$10\,\mathrm{k\Omega}$$)，与2.1.1节的$$R_{\mathrm{MTJ}}(V_{MTJ},\theta)$$式在$$\theta=0$$处给出$$R_P$$的约定一致。磁化状态与电阻之间的连续映射在随机翻转轨迹尚未完全收敛时尤为重要，因为中间磁化态对应的瞬时电阻既不等于$$R_P$$也不等于$$R_{AP}$$。
...
在固定驱动配置 (外加电压$$V$$、电流密度$$J$$、脉冲宽度$$t_w$$) 下重复执行$$N$$次独立仿真，每次仿真使用不同的随机种子以确保热噪声轨迹统计独立。对第$$i$$次仿真，以$$m_z$$的终态判定是否发生翻转 (以反平行态到平行态翻转为例，要求$$m_z$$在脉冲结束后弛豫至$$m_z > +0.5$$的稳定态；实现中取更严的$$m_z \ge 0.8$$)，定义伯努利随机变量：
...
**临界写入电压的脉宽依赖。** 在固定外加面内磁场$$H_x = 200\,\mathrm{Oe}$$条件下 (该实验场与仿真实现所取50 Oe等效偏置的口径关系见2.2.1.2节脚注)，施加脉冲宽度$$t_w = 0.75\,\mathrm{ns}$$、$$1\,\mathrm{ns}$$、$$2\,\mathrm{ns}$$、$$5\,\mathrm{ns}$$的写入脉冲，在每个脉宽下扫描脉冲电压幅值$$V_{\mathrm{SOT}}\in[-1.1, 1.1]\,\mathrm{V}$$，获取完整的电阻-电压滞回回线。每个扫描点先将器件初始化至已知磁化状态，施加单次写入脉冲，随后通过SMU读取MTJ电阻状态以判断翻转是否发生。
...
$$
V_{\mathrm{P\to AP}}(t_w) = 0.82 - 0.17\ln(t_w/\mathrm{ns})\ \mathrm{V},\qquad V_{\mathrm{AP\to P}}(t_w) = -0.79 + 0.18\ln(t_w/\mathrm{ns})\ \mathrm{V},
$$
...
**$$t_w = 0.75\,\mathrm{ns}$$概率翻转特性。** 固定脉冲宽度为$$t_w = 0.75\,\mathrm{ns}$$，对Device A和Device B各以正、负两个方向扫描写入电压，每个幅值独立重复执行50次写入操作，以成功翻转次数占比定义翻转概率$$P_{\mathrm{sw}}$$[^note-cycle-count]。
[^note-cycle-count]: 逐次记录保存于`05_experimental_raw_data`，每个电压点为50行 (循环号、写前电阻、写后电阻、成功标志)，成功标志与电阻跃迁一一对应。46个逐点文件中有44个的文件名所记$$P_{\mathrm{sw}}$$与逐次记录的成功率完全一致，另两点 (Device B $$-840\,\mathrm{mV}$$、Device A $$+1100\,\mathrm{mV}$$) 分别相差$$0.08$$与$$0.04$$，本文拟合沿用文件名值；两点均不在标定所用曲线上。该测量与前述滞回扫描在同一批次内完成 (同一器件、同一连续测试窗口)，构成与NB参数反推直接对应的基准数据。四条$$P_{\mathrm{sw}}(V)$$曲线如图2.14所示，对每条曲线独立进行四参数Sigmoid拟合
$$
...
|:---:|:---:|:---:|:---:|:---:|:---:|:---|
| A | P→AP | 915.5 | 5.45 | 183.5 | 0.969 | $$V\gtrsim 940$$ mV出现回跳平台，拟合受扰 |
| A | AP→P | 894.0 | 22.43 | 44.6 | 0.993 | 干净单段过渡 |
| B | P→AP | 865.9 | 36.54 | 27.4 | 0.968 | 840–860 mV附近出现两段过渡 |
| B | AP→P | 904.5 | 11.26 | 88.8 | 0.995 | 干净单段过渡 |

四组曲线并不完全等价。P→AP方向出现的back-hopping平台和两段过渡提示深过驱区存在宏自旋Sigmoid之外的动力学非理想性；两条AP→P曲线则呈现干净单段过渡，更适合作为行为级接口的标定对象。后续联合模型因此采用Device A、AP→P、$$t_w = 0.75\,\mathrm{ns}$$的Sigmoid拟合结果作为主基准，使Néel-Brown反推、Sigmoid斜率与工艺容差分析共用同一器件、方向与测量窗口。

---

![sMTJ写入特性与Néel-Brown联合概率模型](figs/Chapter02_local_13.png)

**图2.13** sMTJ器件在不同脉冲宽度下的写入特性、Néel-Brown联合概率模型与宏自旋跨脉宽迁移核验。(a)$$t_w = 0.75\,\mathrm{ns}$$、$$1\,\mathrm{ns}$$、$$2\,\mathrm{ns}$$、$$5\,\mathrm{ns}$$条件下测得Device A的电阻-脉冲电压滞回回线，随脉冲持续时间缩短翻转电压窗口逐渐展宽。(b)临界翻转电压$$V_{\mathrm{th}\pm}$$随脉冲宽度的对数依赖关系，空心符号为实验数据点，实线为对数线性拟合$$V = a\mp b\ln(t_w/\mathrm{ns})$$；内嵌注释给出由$$\tau_0 = 1\,\mathrm{ns}$$先验反推得到的两方向Néel-Brown参数$$(\Delta, V_{c0}, \tau_{\mathrm{ret}})$$。(c)基于反推NB参数构建的二维联合翻转概率分布$$P_{\mathrm{sw}}(V, t_w)$$热力图(P→AP方向)，紫色等概率轮廓在低概率区可读，白色等概率轮廓在高概率区可读，黑色虚线为50%等概率轨迹即$$V_{\mathrm{th}}(t_w)$$；空心圆(Device A)与三角(Device B)标示两器件的滞回提取点，与50%轨迹吻合。(d)跨脉宽迁移核验(AP→P方向)，红色空心圆为以0.75 ns单点标定$$\theta_{\mathrm{SH}}$$的宏自旋仿真50%阈值(Cayley积分器、自热开启，误差棒为$$1\sigma$$统计分辨率)，青色虚线与方块为(b)的实验对数线性律及其测量脉宽，琥珀色星号为0.75 ns Sigmoid标定锚点，红色实线为仿真数据自身的对数线性拟合；仿真对数斜率$$87\,\mathrm{mV}$$约为实验$$175\,\mathrm{mV}$$之半，阈值偏差随脉宽增至$$+40.5\%$$。

---

![Sigmoid测量与Néel-Brown外推对比](figs/Chapter02_local_14.png)

**图2.14** $$t_w = 0.75\,\mathrm{ns}$$、$$H_x = 200\,\mathrm{Oe}$$条件下50次重复Sigmoid测量与C2C-修正后的Néel-Brown模型对比。(a)Device A P→AP：实测在$$V \gtrsim 940\,\mathrm{mV}$$出现back-hopping回跳平台。(b)Device A AP→P：干净单段过渡($$R^2 > 0.99$$)，作为主基准曲线。(c)Device B P→AP：840–860 mV附近出现两段过渡。(d)Device B AP→P：干净单段过渡($$R^2 > 0.99$$)。空心符号为实验数据点(误差线为二项分布的Wilson 95%置信区间)，实线为C2C-修正NB曲线 (数学上等价于四参数Sigmoid拟合)，各面板内嵌注释给出$$\eta_c = \beta_s/\beta^{\mathrm{NB}}$$与该曲线的物理特征。Sigmoid拟合对四条曲线的$$V_{\mathrm{th}}$$预测精度均优于+7.4%，但未修正NB预测的斜率(约8 V⁻¹)普遍低于实测数倍，必须以$$\eta_c$$因子作C2C修正方能重现实测分布陡度。

---

![同批次器件间Néel-Brown参数一致性对比](figs/Chapter02_local_15.png)

**图2.15** Device A 与 Device B 两个器件的Néel-Brown参数一致性对比。(a)正向(P→AP)临界翻转电压$$V_{\mathrm{th}+}$$随脉冲宽度$$t_w$$的对数线性依赖；Device A (红色圆点) 与Device B (紫色三角) 数据点近似落在同一条对数直线上，两器件的$$\Delta$$与$$V_{c0}$$拟合值在5%–15%范围内一致。(b)以$$\tau_0 = 1\,\mathrm{ns}$$先验反推得到的两方向热稳定性因子$$\Delta$$柱状图，P→AP (红色) 与AP→P (蓝色) 方向在同一器件上数值接近(器件A的两方向$$\Delta$$分别为5.15与4.91、器件B分别为4.46与4.95)，对应零温临界电压$$V_{c0}$$分别为884 mV (器件A) 与876 mV (器件B)，证实sMTJ作为概率单元具备良好的器件间均匀性，为后续阵列级建模与工艺容差分析提供同批次基线参考。
...
|:---:|:---:|:---:|:---:|
| P→AP | 172 mV | 120 ns | 172 ns |
| AP→P | 175 mV | 94 ns | 135 ns |
...
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| A | P→AP | 870 mV | 915.5 mV | +5.2% | 8.08 V⁻¹ | 183.5 V⁻¹ | 22.7 |
| A | AP→P | 843 mV | 894.0 mV | +6.0% | 7.94 V⁻¹ | 44.6 V⁻¹ | 5.6 |
| B | P→AP | 861 mV | 865.9 mV | +0.6% | 7.06 V⁻¹ | 27.4 V⁻¹ | 3.9 |
| B | AP→P | 842 mV | 904.5 mV | +7.4% | 8.02 V⁻¹ | 88.8 V⁻¹ | 11.1 |
...
主基准曲线 (Device A、AP→P) 实测$$\eta_c > 1$$的主体可由热辅助进动区的动力学相干性解释，这一归因经2.2节宏自旋求解器直接核验：在标定工作点对过渡区做加密Monte Carlo扫描并以同一四参数Sigmoid拟合，sLLG给出斜率$$\beta_s^{\mathrm{sLLG}} = 33.8\pm13.5\,\mathrm{V^{-1}}$$ (95%置信区间)，显著高于NB外推的$$7.94\,\mathrm{V^{-1}}$$，与实测$$44.6\,\mathrm{V^{-1}}$$在区间内一致，按对数计弥合了$$\eta_c$$差距的约84%[^note-eta-sllg]。$$t_w = 0.75\,\mathrm{ns}$$位于纯热激活 (长脉宽、低电压) 与纯进动翻转 (短脉宽、高电压) 之间的机制交叉区，翻转事件获得部分相干性，C2C分布因此窄于纯热激活Gumbel极限；该收窄在单畴图像内即可产生，Néel-Brown解析式之所以未能刻画，是因为速率律只保留了能垒统计而丢弃了进动动力学。就主基准曲线而言，亚畴协同跃迁等超出宏自旋自由度的机制只需解释残余的少部分斜率差；表2.9中$$\eta_c$$更高的其余三条曲线能否同样归因，尚待逐条仿真核验。P→AP方向同时出现回跳平台与两段过渡而AP→P方向基本不出现的事实，一个自然的候选解释是参考层杂散场的非对称调制；杂散场注入检验的结果分为两层：z向杂散场使两方向阈值线性劈裂，$$H_z\approx25\,\mathrm{Oe}$$即产生与主基准器件实测同量级、同方向的阈值不对称，但回跳平台的方向选择性与两段过渡均未随之出现——阈值不对称可由杂散场定量解释，P→AP特有的回跳形态与两段过渡则仍需宏自旋之外的机制[^note-e5-stray]。
[^note-e5-stray]: 注入设置沿用图2.11口径 (Cayley积分器、自热开启)，每方向、每场值11点×150条轨迹，两方向分别以初态与电流极性实现 (AP→P取$$-I$$、P→AP取$$+I$$)。$$H_z=0/25/50/100\,\mathrm{Oe}$$下阈值劈裂$$\Delta V_{\mathrm{th}}=V_{\mathrm{th}}^{\mathrm{P\to AP}}-V_{\mathrm{th}}^{\mathrm{AP\to P}}=-4/+27/+74/+144\,\mathrm{mV}$$，各值的MC统计分辨率约$$11$$–$$16\,\mathrm{mV}$$ ($$H_z=0$$对照组由此在分辨率内对称)；$$25\,\mathrm{Oe}$$量级即与主基准器件Device A的实测两方向差相当 (表2.6中Sigmoid口径$$21.5\,\mathrm{mV}$$、滞回截距口径约$$30\,\mathrm{mV}$$)，且劈裂方向一致 (P→AP更高)，Device B两方向差符号相反，与器件间残余场取向差异相容；两方向的过驱平台带在各场值下均处于$$0.68$$–$$0.83$$，未出现与实测对应的一致方向选择性，各扫描曲线亦均为单段上升进入平台、无两段结构。
[^note-eta-sllg]: 核验设置与表2.9的$$\eta_c$$口径一致：Device A、AP→P、$$t_w=0.75\,\mathrm{ns}$$、保模长Cayley积分器、自热开启，过渡区13个电压点、每点$$10^3$$条独立轨迹，四参数logistic拟合 ($$R^2=0.995$$)，区间取拟合协方差的95%置信半宽。斜率阶梯为$$\beta_s^{\mathrm{NB}}=7.94\to\beta_s^{\mathrm{sLLG}}=33.8\to\beta_s^{\mathrm{meas}}=44.6\,\mathrm{V^{-1}}$$，对数份额$$\ln(33.8/7.94)/\ln(44.6/7.94)\approx0.84$$，置信下界对应约0.55。同一扫描还复核了图2.11过驱平台的存在及其稳健性：$$0.97$$–$$1.24\,\mathrm{V}$$区间仿真$$P_{\mathrm{sw}}$$平坦于$$0.70$$–$$0.78$$，抽检$$1.09/1.55\,\mathrm{V}$$两点对脉后判定窗$$3.25/10/50\,\mathrm{ns}$$不敏感，$$1.55\,\mathrm{V}$$处平台约$$0.81$$，与图2.11在更宽电流范围报告的$$0.75$$–$$0.85$$平台在统计区间内一致，说明平台源于脉冲期间的过驱进动随机化而非读出判定口径；实测AP→P至其数据上限$$\approx1.02\,\mathrm{V}$$已饱和至1，仿真平台反而与实测P→AP方向的回跳平台形态相近，该模型-实验分歧与正文方向非对称问题同源；杂散场注入检验 (见正文同段) 未再现平台的方向选择性，该分歧仍需宏自旋之外的机制解释。
...
代入Device A、AP→P、$$t_w = 0.75\,\mathrm{ns}$$、$$\tau_0 = 1\,\mathrm{ns}$$、$$\Delta = 4.91$$：$$\ln(0.75/0.693) \approx 0.0792$$，$$\xi = 0.0792/4.91 \approx 0.0161$$，故
...
[^note-eta-fit]: 5.34为Monte Carlo数值拟合所得Device A、AP→P方向值；以表2.8解析参数代入$$\beta_s^{\mathrm{meas}}/\beta_s^{\mathrm{NB,\,analytic}} = 44.6/7.94 = 5.62$$，两者差异源于MC实现对NB拟合的轻度有限$$N$$偏差，不影响下游分析结论。

$$
\beta^{\mathrm{eff}}(\mathrm{CV}_\Delta) = \eta_c \cdot \mathcal{F}(\mathrm{CV}_\Delta) \cdot \beta_{\mathrm{NB}}^{\mathrm{fit}}
$$

图2.16验证了上述解析判断：在PDK基线下，D2D的$$\Delta$$离散对单器件Sigmoid斜率的修正很弱，C2C收窄因子仍是决定实测斜率的主导项。PDK-Brinkman反推、解析灵敏度估计与Monte Carlo平均曲线三者给出一致方向。

在$$\Delta$$单参数投影口径下，工艺裕度评估见表2.11：只要工艺$$\mathrm{CV}_\Delta$$维持在PDK基线附近，阵列平均Sigmoid斜率基本保持；即便工艺显著恶化，斜率退化也缓慢。

**表2.11** 工艺裕度 ($$\Delta$$单参数投影口径)：阵列$$\beta^{\mathrm{eff}}$$相对单器件$$\beta_s$$的保持度与所需$$\mathrm{CV}_\Delta$$上限(Device A、AP→P、$$t_w = 0.75\,\mathrm{ns}$$)
...
在仅将工艺失配投影为$$\Delta$$单参数扰动的NB传递函数中，PDK基线对应单器件$$\beta_s$$保持率99.7%。这说明热稳定因子离散本身对该纳秒工作点的晶圆平均斜率退化较弱；若进一步讨论固定端电压写入，则还需把$$R_{\mathrm{SOT}}$$引起的电压到电流换算漂移以及磁性参数对动力学阈值的共同影响纳入器件级求解。故工艺优化不能仅凭$$\Delta$$单参数图判断实测Sigmoid局部形状，仍需结合下述宏自旋失配仿真共同评估；其中阈值的器件间均匀性可由该仿真直接给出，而$$\eta_c$$本身的器件间均匀性受限于单器件斜率所需的采样量，留待专门的高采样实验。

本节基于PDK标称失配参数推导得到的$$P_{\mathrm{sw}}$$响应是一条统计意义下的等效Sigmoid曲线，其形状由$$(\mathrm{CV}_\Delta, \eta_c)$$两个标量参数决定，无法刻画实测中观察到的back-hopping平台、两段过渡等单器件级畸变。换言之，实测$$P_{\mathrm{sw}}$$相对PDK推导基线存在的局部漂变要大于PDK单参数化所给出的展宽幅度。2.2节的sLLG求解器可再现其中的动力学畸变——back-hopping平台与C2C斜率主体已在2.3.4节经仿真核验；亚畴协同跃迁超出宏自旋自由度，两段过渡与回跳平台的方向选择性亦未在仿真中再现，此类器件级畸变在系统仿真前端以实测曲线为准注入，其微观归因留待微磁分析。
...
为检验上述$$\Delta$$单参数投影是否遗漏端电压驱动下的器件级展宽，本工作进一步将同一PDK失配预算注入宏自旋求解器：由$$R_P$$残余失配采样$$D$$与$$D_{\mathrm{elec}}$$，由$$t_f$$、$$M_s$$与TMR代理采样自由层厚度、饱和磁化和各向异性参数，并将$$R_{\mathrm{SOT}}$$失配映射到$$R_W$$。该设置在固定$$|V_{\mathrm{SOT}}|$$下尤其关键，因为$$R_W$$变化会同步改变每个样本实际获得的$$|I_{\mathrm{SOT}}|$$与SOT自热功率。图2.17显示标称器件在实测阈值附近保留清晰的Sigmoid上升段，而同一PDK预算下的晶圆平均曲线明显趋缓：$$P_{\mathrm{sw}}$$自$$0.25$$升至$$0.5$$所需的电压跨度由$$51.5\,\mathrm{mV}$$增至$$198.2\,\mathrm{mV}$$，过渡区展宽约$$3.85$$倍，同时在标称阈值以下出现翻转尾部[^note-e7-macrospin]。两种投影口径的差距由此界定：图2.16的$$\Delta$$单参数传递函数在PDK基线处给出$$\mathcal{F}=0.997$$，即晶圆平均斜率相对单器件几乎不变，而固定端电压下的完整失配注入给出数倍展宽。该差距来自$$\Delta$$投影未纳入的两条通道：磁性与几何失配对动力学阈值$$V_{c0}$$本身的横向搬移 ($$\Delta$$投影中$$V_{c0}$$固定于标称值)，以及$$R_{\mathrm{SOT}}$$失配引起的端电压-驱动电流换算漂移；逐器件扫描直接测得该横向搬移的幅度[^note-e7-devices]。故表2.11的工艺裕度应理解为$$\Delta$$单参数投影口径下的结果，阵列级Sigmoid斜率的预算须在器件级求解器中按端口条件注入几何、磁性与沟道电阻的联合失配。
[^note-e7-macrospin]: 统计规模为16个D2D失配器件×每点32次热噪声试验 (池化512次)，标称曲线的Wilson 95%半宽自早期6×8设置的$$\pm14\%$$收紧至$$\pm4.3\%$$，展宽幅度远超该带宽；晶圆平均曲线是16个器件级估计的平均，其不确定度以器件间散布为主，图中阴影与下文引用的$$\pm$$值均仅给出二项分量。展宽以无拟合的电压跨度度量，原因是晶圆平均曲线在插图窗口内未达饱和 (最高$$0.633$$)，四参数logistic的基线与幅度近简并、斜率随基线约束在$$5.6$$–$$9.1\,\mathrm{V^{-1}}$$间漂移；以物理约束$$y_0\ge0$$拟合时标称$$\beta_s=30.2\pm4.9\,\mathrm{V^{-1}}$$、晶圆平均$$9.1\pm4.9\,\mathrm{V^{-1}}$$ ($$R^2$$分别为$$0.996$$与$$0.988$$)，与跨度度量给出的展宽倍数同向。标称曲线原始50%穿越点$$896\pm7\,\mathrm{mV}$$与2.3.2节实测拟合锚点$$894\,\mathrm{mV}$$一致，其logistic斜率$$30.2\,\mathrm{V^{-1}}$$亦与2.3.4节加密扫描的$$\beta_s^{\mathrm{sLLG}}=33.8\pm13.5\,\mathrm{V^{-1}}$$相符，两项构成本节失配注入与主基准之间的自洽核验。晶圆平均曲线穿越点上移至$$965\pm14\,\mathrm{mV}$$；低阈值样本在$$0.62$$–$$0.78\,\mathrm{V}$$贡献$$0.04$$–$$0.26$$的翻转尾部抬高低压端，与穿越点的上移合并，使中段趋缓。上移的成因由正文所引的逐器件核验给出。
[^note-e7-devices]: 以同一采样器、同一主种子与反演配对重建这16个失配器件并逐个扫描 (10个电压点×48条轨迹)。该扫描先给出一项无模型的自洽核验：把16条实测曲线直接平均，得50%穿越点$$974\,\mathrm{mV}$$、$$0.25$$–$$0.5$$跨度$$201\,\mathrm{mV}$$，与图2.17晶圆平均在独立电压网格与不同试验数下测得的$$965\,\mathrm{mV}$$与$$198\,\mathrm{mV}$$相符，说明可以直接以器件总体解释该曲线。阈值总体为均值$$892\,\mathrm{mV}$$ (标称$$896\,\mathrm{mV}$$)、标准差$$140\,\mathrm{mV}$$、变异系数$$15.7\%$$、极差$$691$$–$$1148\,\mathrm{mV}$$；其中1个器件在整个扫描区间内$$P_{\mathrm{sw}}$$于$$0.15$$–$$0.52$$间无序起伏、扫描上端仅$$0.33$$，未建立可用的确定性窗口，其穿越点为噪声产物，剔除后统计量仅变为$$900\pm141\,\mathrm{mV}$$、变异系数$$15.6\%$$。关键对比在于：阈值散布的标准差约为标称过渡区宽度 ($$51.5\,\mathrm{mV}$$) 的$$2.7$$倍，而单器件自身的过渡仍然陡峭 ($$0.25$$–$$0.5$$跨度均值$$57.9\pm16.0\,\mathrm{mV}$$、极差$$37$$–$$92\,\mathrm{mV}$$，与标称同量级)，故晶圆平均的展宽由阈值横向散布承载，而单器件的Sigmoid特征并未退化。穿越点上移的成因同样由此明确：单器件曲线饱和于约$$0.75$$，读出电平$$0.5$$落在其拐点 ($$L/2\approx0.375$$) 之上的凹区，按Jensen不等式，阈值弥散使平均曲线在阈值均值处低于$$0.5$$，须待约三分之二的器件进入各自平台才达到$$0.5$$，平均曲线的穿越点因而高出器件阈值均值约$$80\,\mathrm{mV}$$。

---

![宏自旋工艺失配Monte Carlo下的Psw端电压扫描](figs/Chapter02_local_17.png)

**图2.17** 工艺失配下的端电压写入概率Monte Carlo曲线，$$t_p = 0.75\,\mathrm{ns}$$、$$V_{\mathrm{MTJ}} = 0$$。主图将图2.11的$$|I_{\mathrm{SOT}}| = 300\text{-}3500\,\mu\mathrm{A}$$宽谱扫描按标称$$R_W$$映射为固定$$|V_{\mathrm{SOT}}|$$轴，插图放大$$800\text{-}1400\,\mu\mathrm{A}$$阈值窗口对应的电压区间。蓝色圆线为标称宏自旋曲线 (同样池化512次热噪声试验)，红色方线为PDK失配样本的晶圆平均，阴影为按池化试验数给出的Wilson 95%区间 (对失配平均曲线仅含二项分量)，绿色虚线为同批次实测$$V_{\mathrm{th}} = 894\,\mathrm{mV}$$参考线；工艺样本采用16个D2D失配器件、每点32次热噪声试验 (池化512次，保模长Cayley积分器、自热开启)。插图中标称曲线在阈值附近仍呈Sigmoid上升，失配平均曲线则在标称阈值以下即出现翻转尾部、在阈值以上推迟饱和，两端合并使中段变浅；这反映不同器件阈值横向散布以及$$R_{\mathrm{SOT}}$$引起的端电压到驱动电流换算漂移，而非单器件Sigmoid特征消失。
...
固定操作点为主基准Device A, AP→P, $$t_w = 0.75\,\mathrm{ns}$$；选取PDK基线、中等恶化和严重恶化三档$$\mathrm{CV}_\Delta$$作为工艺代表，并在对数网格上重复估计
...
**硬件端：sMTJ作为Bernoulli随机源的运行时采样数。** 硬件端每次采样对应器件的一次物理写入-读取循环，受限于能耗0.78 pJ/次、延迟0.75 ns/次与器件耐久性。该循环成立的前提是读出不改写被采样的比特，这一前提可由求解器直接检验：以2.3.1节的10 mV读偏压保持10 ns、SOT电流置零，300次读取中未观测到扰动；提高偏压至$$0.2\,\mathrm{V}$$仍未观测到扰动，至$$0.4\,\mathrm{V}$$才出现 (13/300)，$$0.6\,\mathrm{V}$$以上几乎必然翻转。按驱动隧穿电流计，读出工作点的$$1.0\,\mu\mathrm{A}$$与首次观测到扰动的$$46\,\mu\mathrm{A}$$相差约46倍。关断对照表明该扰动由隧穿电流的自旋转移力矩驱动，单独保留VCMA不足以引发扰动；在所测读偏压极性下，受威胁的只是初态$$m_z\approx-1$$的势阱，初态$$m_z\approx+1$$在全部偏压下均未被扰动[^note-e8b-read]。在概率表征阶段$$K$$决定$$P_{\mathrm{sw}}$$曲线拟合的置信度(2.3.2节采用$$K = 50$$)，在随机比特流算术中$$K$$决定数值精度与比特流长度，在PBNN推断中$$K$$决定单次前向传播中每个节点的采样次数，$$K$$的压缩直接映射到每概率输出的能耗与吞吐率。与建模端不同，硬件采样问题存在可解析的精确解：$$\hat p_K = K^{-1}\sum_{i=1}^K X_i$$中的$$X_i\sim\mathrm{Ber}(p)$$独立同分布，$$K\hat p_K\sim\mathrm{Bin}(K, p)$$，覆盖率可由Binomial CDF直接求出。下面并列对比三条分析路径(精确Binomial、Monte Carlo、CLT近似)以明确各自的有效性边界。
[^note-e8b-read]: 设置为$$I_{\mathrm{SOT}}=0$$、仅在MTJ两端施加读偏压并保持$$10\,\mathrm{ns}$$ (远长于实际读出窗口)，STT与VCMA两条通道同时开启，自热开启，每点300次独立试验，判据为$$m_z$$越过赤道进入另一势阱；只扫描正向读偏压，该极性对$$m_z\approx-1$$势阱是最不利情形。初态$$m_z\approx-1$$的逐偏压结果为$$0/300$$ ($$0.01\,\mathrm{V}$$)、$$0/300$$ ($$0.2\,\mathrm{V}$$)、$$13/300$$ ($$0.4\,\mathrm{V}$$)、$$299/300$$ ($$0.6\,\mathrm{V}$$)、$$300/300$$ ($$0.8$$与$$1.0\,\mathrm{V}$$)；初态$$m_z\approx+1$$在$$0.01$$至$$1.0\,\mathrm{V}$$全部为$$0/300$$。$$10\,\mathrm{mV}$$处的零观测对应每次读$$1.3\%$$的95%置信上界，该上界受试验数限制。扰动起始只被括在$$(0.2,\,0.4]\,\mathrm{V}$$区间内，故数据直接支撑的裕度为电流$$1.0$$对$$21\,\mu\mathrm{A}$$、约21倍，正文引用的46倍以首次观测到扰动的偏压为界。通道对照在$$0.8\,\mathrm{V}$$、初态$$m_z\approx-1$$下进行：单留VCMA为$$0/120$$、单留STT为$$119/120$$，与两通道同开的$$300/300$$一致，说明VCMA不提供力矩、单独不足以引发扰动；该对照位于饱和偏压处，因而不能分辨VCMA能垒调制对扰动阈值本身的影响。初态依赖源自自旋转移力矩的极化方向：正向读偏压把磁化推向$$+z$$，故只有$$m_z\approx-1$$的势阱受威胁。
...
在工艺层面，本章的主结论是纳秒概率写入对$$\Delta$$扰动本身低灵敏。几何、界面和材料涨落经$$\Delta$$投影只产生有限的阵列展宽，但在固定端电压的端口条件下，同一失配预算会把过渡区展宽数倍，故阵列级斜率预算须在器件级求解器中评估。运行时采样则由精确Binomial覆盖率决定：Sigmoid中段需要更多重复写入，饱和区可以压缩样本数；这一规则直接服务于后续PBNN和概率权重调度。
