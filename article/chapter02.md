...
本节把2.1节的概率翻转抽象落到可复现实算的物理引擎上。sMTJ的C2C随机性来自热涨落驱动下的磁化动力学；若仿真链路不能同时保留热噪声、自热反馈、温度依赖材料参数与端口电学耦合，后续由实验拟合得到的Sigmoid参数就缺少可解释的物理来源。

本文采用宏自旋sLLG模型作为主求解层级。该选择服务于本章主线：研究对象是80 nm级、接近单畴的sMTJ概率单元[^ref-kittel-domain]，关注量是翻转概率、能耗和阵列级参数传递。建立一条从有效场、随机热场、温度反馈到$$P_{\mathrm{sw}}$$统计量的闭合链路，并将其实现为后续实验校准和系统评估都可调用的仿真平台。
...
图2.5(a)–(c)汇总了$$M_s(T)$$、$$K_i(T)$$与$$\eta(T)$$的温度漂移。对本章模型而言，关键结论是$$K_i$$随温度衰减得比$$M_s$$更快：自热首先削弱有效PMA场和能垒，再通过热噪声幅度共同改变临界区的翻转概率。
...
基于2.2.4节所述平台，本文将器件行为输出压缩为两类对后续章节真正有用的观测量。第一类是单条轨迹的$$m_z(t)$$与由TMR模型换算得到的$$R_{\mathrm{MTJ}}(t)$$，用于确认仿真轨迹是否呈现合理的翻转事件、读出电阻跳变和弛豫过程。图2.8给出代表性样本，用于检验物理过程是否自洽。为补充$$m_z(t)$$标量视图，平台还可同时记录极角与方位角并重构完整磁化矢量，如图2.9的三维轨迹。
...
图2.15验证了上述解析判断：在PDK基线下，D2D展宽对单器件Sigmoid斜率的修正很弱，C2C收窄因子仍是决定实测斜率的主导项。PDK-Brinkman反推、解析灵敏度估计与Monte Carlo平均曲线三者给出一致方向。
...
图2.21(a)(b)与表2.15把不同构型的容量指标集中列出。正文只保留三条设计判断：异质输入投影把低势垒节点群变成一组可叠加的衰落滤波器，因此显著优于广播同质基线；独立节点的线性记忆最终受基函数秩限制，单纯增加节点数不能无限提高容量；环形延迟线可以把容量转向线性记忆，但会牺牲非线性处理能力，Mackey-Glass混沌预测上的构型排序随之反转 (延迟反馈最优、环形延迟线最差)。由此，储备池设计的核心不在于堆节点数，而在于按任务在记忆深度和非线性之间分配结构。
...
在模型与平台层面，本章的创新点是把端口解耦、随机磁化动力学和工艺标定放进同一条链。T型电路负责把$$V_{\mathrm{MTJ}}$$与$$I_{\mathrm{SOT}}$$解析解耦，宏自旋sLLG负责保留SOT、VCMA、热涨落与自热反馈，Cayley积分器保证随机轨迹在大规模Monte Carlo中保持模长稳定。vgsot-sim由此成为把$$m_z(t)$$轨迹、$$P_{\mathrm{sw}}$$曲线、工艺失配和采样预算统一到同一参数口径下的实验前端。
...
在工艺与采样层面，本章的主结论是纳秒概率写入对$$\Delta$$扰动天然低灵敏。几何、界面和材料涨落仍会进入$$\Delta$$预算，在本文工作点上主要表现为有限的阵列横向展宽。运行时采样则由精确Binomial覆盖率决定：Sigmoid中段需要更多重复写入，饱和区可以压缩样本数；这一规则直接服务于后续PBNN和概率权重调度。
