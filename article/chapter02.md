...
以线性记忆容量$$\mathrm{MC}=\sum_k \mathrm{corr}^2(\hat u[t-k],\,u[t-k])$$为度量，异质投影节点群在$$n=25\to200$$时给出$$\mathrm{MC}=6.3\to8.1$$，较广播基线提升约14倍，且$$\mathrm{MC}_k$$随延迟单调衰减、$$\mathrm{MC}_1$$接近1，衰落记忆特征清晰 (图2.21(a)(b))。容量在$$n\approx100$$后趋于饱和：相互独立的节点构成滤波器组，线性记忆受其基函数秩而非节点数限制，欲使容量随$$n$$持续增长需引入节点间耦合或延迟反馈。非线性容量则已可观：延迟奇偶校验parity-2/3/4准确率达1.00/1.00/0.91，按一、二阶Legendre基分解的信息处理容量约6.6，NARMA-10任务 ($$n=200$$) 归一化均方根误差约0.55；把同一节点经掩码输入与延迟反馈时间复用为虚拟节点群 (2.4.4节所述另一构型)，总信息处理容量相当但配比不同——线性记忆更低而二阶容量更高，两种构型的基准汇总见表2.15。

**表2.15** 两种最小储备池构型的基准容量 ($$n=100$$节点/虚拟节点，平均场态，固定种子)
...
**图2.21** RTN节点群储备池的最小验证 (平均场态，固定种子，全部数值可由基准脚本确定性复现)。(a) 逐延迟记忆容量$$\mathrm{MC}_k$$：异质$$W_{\mathrm{in}}$$节点群 ($$n=100$$，蓝) 对比广播同质基线 (灰)，后者无论节点数多少都坍缩到$$\mathrm{MC}=0.58$$；(b) 总记忆容量随节点数的变化，$$n\approx100$$后趋于饱和，即独立节点滤波器组的秩上限；(c) 二值器件态下每节点平均$$R$$个器件的记忆容量 (红)，对照平均场极限 (蓝虚线)——器件平均是硬件化的主要开销。
