# 第二章 三端SOT-sMTJ器件建模、仿真与实验验证

面向后续阵列级概率计算与系统级代价评估，本章需要先把三端SOT-sMTJ从器件端口、磁化动力学、统计翻转行为和实验标定四个层面接成同一套可调用模型。为此，2.1节从T型电路与SOT-VCMA联合驱动出发建立概率翻转的器件级抽象，2.2节以含热噪声、自热反馈和温度依赖材料参数的sLLG求解流程说明该抽象的物理来源与仿真实现，2.3节再用300 mm工艺平台上的单器件测试与工艺波动分析校准Néel-Brown和Sigmoid参数，最终在2.4节归纳后续伊辛机与PBNN章节可直接继承的工程接口。
...
![三端SOT-sMTJ器件结构与T型等效电路网络](figs/fig_01_t_circuit.png)
...
![面向概率计算的sMTJ行为级模型分层架构](figs/fig_02_behavioral_layers.png)
...
![sMTJ温度效应和非理想性建模总览](figs/fig_03_thermal_nonidealities.png)
...
![瞬态热演化仿真结果](figs/fig_04_thermal_transients.png)
...
![温度依赖材料参数与TMR偏压响应](figs/fig_05_material_params.png)
...
![自热反馈对纯SOT翻转轨迹的影响](figs/fig_06_self_heating_trajectory.png)
...
![vgsot-sim三层架构示意](figs/fig_07_vgsot_sim_architecture.png)
...
![单次磁化矢量在单位球面上的三维轨迹](figs/fig_09_3d_trajectory.png)
...
![Monte Carlo Psw扫描结果](figs/fig_10_ser_mc.png)
...
在基于MRAM的概率计算体系中，热激活驱动的随机磁化翻转过程构成伯努利分布的物理随机源，器件的翻转概率$$P_{\mathrm{sw}}(V, t)$$不再作为误差来源，而是作为可控计算资源被主动利用[^ref-borders-factorization]。该计算范式对器件提出特定工作要求，例如，在亚阈值或近阈值偏置条件下翻转概率应呈现稳定、连续且可精确建模的电压响应，器件还应具备足够的耐久性与晶圆级工艺一致性以支撑阵列规模下的统计均匀性。

---

![SOT-MTJ器件实验表征综合图](figs/fig_11_experimental_setup.png)
...
![sMTJ写入特性与Néel-Brown联合概率模型](figs/fig_12_exp_nb_summary.png)
...
![Sigmoid测量与Néel-Brown外推对比](figs/fig_13_psw_sigmoid.png)
...
![同批次器件间Néel-Brown参数一致性对比](figs/fig_14_device_consistency.png)
...
![MC采样数对D2D传递函数估计量的影响](figs/fig_16_mc_sampling_sensitivity.png)
...
![sMTJ硬件Bernoulli采样可靠性分析](figs/fig_17_hw_sampling_reliability.png)
