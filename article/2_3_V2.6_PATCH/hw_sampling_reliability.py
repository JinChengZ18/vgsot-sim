...
# ═════════════════════════════════════════════════════════════════════════
# 1. Exact Binomial coverage
# ═════════════════════════════════════════════════════════════════════════
def exact_coverage(K, p, eps):
    """Exact P(|k/K − p| < eps),  k ~ Bin(K, p).  Strict inequality."""
    K = int(K)
    ...
    if k_lo > k_hi:
        return 0.0
    return stats.binom.cdf(k_hi, K, p) - stats.binom.cdf(k_lo - 1, K, p)

def coverage_array(K_arr, p, eps):
    return np.array([exact_coverage(K, p, eps) for K in K_arr])

# ═════════════════════════════════════════════════════════════════════════
# 2. Persistent-monotone  K_req  via iterative bisection
# ═════════════════════════════════════════════════════════════════════════
def find_K_req(p, eps, conf=0.95):
    """Smallest K* such that coverage(K') >= conf for all K' >= K*."""
    z = stats.norm.ppf((1 + conf) / 2)
    K_hi = max(int(np.ceil(z**2 * p * (1 - p) / eps**2) * 4), 200)
    while exact_coverage(K_hi, p, eps) < conf:
        K_hi *= 2
    covs = coverage_array(np.arange(1, K_hi + 1), p, eps)
    min_cov = np.minimum.accumulate(covs[::-1])[::-1]
    ok = np.where(min_cov >= conf)[0]
    if len(ok) == 0:
        raise RuntimeError("Persistence target unreachable")
    return int(ok[0] + 1)

# ═════════════════════════════════════════════════════════════════════════
# 3. Monte Carlo coverage estimator
# ═════════════════════════════════════════════════════════════════════════
def mc_coverage(K, p, eps, M, rng):
    """Empirical coverage from M Binomial(K,p) replicates.

    Uses integer comparison k_lo <= k <= k_hi rather than the naive
    float-space |k/K - p| < eps, which would otherwise miscount the
    band-edge integers by ~2^-52 rounding (e.g. |0.40 - 0.50| evaluates
    to 0.09999999999999998 < 0.10 in IEEE 754 double precision, spuriously
    including the edge integers k = 40 and k = 60 at K = 100, p = 0.5,
    eps = 0.10 and biasing the MC coverage estimate by ~2%).
    """
...
def find_K_req_mc(p, eps, M, rng, conf=0.95):
    """MC analogue of find_K_req via persistence on empirical coverage."""
    z = stats.norm.ppf((1 + conf) / 2)
    K_hi = max(int(np.ceil(z**2 * p * (1 - p) / eps**2) * 4), 200)
    while mc_coverage(K_hi, p, eps, M, rng) < conf + 0.01:
        K_hi *= 2
        if K_hi > 10_000_000:
            return None
    covs = np.array([mc_coverage(K, p, eps, M, rng)
...
ax_a.legend(loc="lower center", fontsize=7.5, ncol=2)
...
ax_c.text(2.3, 0.952, r"95% target", color=CHARCOAL,
...
ax_d.text(12, 0.0108, r"1% RMSE", color=AMBER, fontsize=8,
          ha="left", va="bottom")

# Annotate the exact value
ax_d.text(0.35, 0.05,
          rf"$\mathrm{{cov}}_{{\rm exact}}(K{{=}}100,p{{=}}0.5,"
          rf"\varepsilon{{=}}0.10)$" "\n"
          rf"$= {cov_true:.4f}$",
          transform=ax_d.transAxes, fontsize=8.5, va="top", ha="left",
          bbox=dict(boxstyle="round,pad=0.35",
                    facecolor=THU_TINT, edgecolor=THU_PALE, alpha=0.93))
...
ax_e.legend(loc="upper right", fontsize=7.3, ncol=1)
...
ax_f.text(1.0, 110, r"§2.3.3:  $K = 100$",
          color=CHARCOAL, fontsize=8, ha="left", va="bottom")
