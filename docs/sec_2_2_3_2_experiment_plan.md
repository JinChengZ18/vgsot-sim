...
| C2 | SC5 Boltzmann 无需修正 | 5 | 被实现证伪(探针) | B | ✅ **定案：不被证伪**（全部三积分器 60ns 收齐，合并产物 `teff_full_merged.{json,png}`）：发布 Cayley dt→0 截距 0.9786±0.0144（-1.5σ）、斜率 +1.2σ → 稳态 1–2% 内即 Boltzmann；真中点对照 0.9788±0.0136 与之不可区分（两者共享的 ~-2% 截距为估计器窗口残差量级）；Euler-球坐标 0.4920±0.0101（**-50σ**）= T/2 缺陷（机制见下）。探针 1.8–2.0× 与 pilot 0.83 均为伪影 |
| C10 | 归一化注入稳态偏置 | 2 | depends | B | ✅ **了结：无可测偏置**——cayley_explicit（含每步归一化）与 cayley_true_midpoint 的 dt→0 截距 0.9786 vs 0.9788，1.5% 精度内同一 |
...
**B 的重大新发现（60ns 全量定案）**：Euler-球坐标积分器的平衡有效温度锁死在 **T_eff/T ≈ 0.50（四档 dt 0.05–0.4ps 给 0.488/0.501/0.505/0.507，dt-无关）**——**恰好一半**。机制已定：`dynamic_switching.py` 的 dθ/dt 把 Stratonovich sLLG 当 Itô 积分（左端点），**结构性缺失 Wong-Zakai 噪声诱导漂移项 ∝ D·cotθ**（极点附近的 D/θ 熵斥力，负责建立相空间 sinθ Jacobian）。解析预言：矩估计 ⟨θ²⟩ 减半 → T_eff/T=0.5 精确（实测 ✓）；斜率估计因多出 −½ln u 项落在 ~0.55–0.6（实测 0.559–0.575 ✓）。现有 `test_integrator_consistency.py`（固定噪声场单步 RHS 检验）原理上无法捕捉此类系综极限缺陷——只有平衡分布测试能，恰是 C8 批评的最佳论据。**影响**：fig2.11 SER MC 默认 euler 路径 → 其热统计实际在 T/2 下采样，热展宽系统性低估；修复选项 (a) fig2.11 换 `integrator="cayley"` 重跑（已验证 T_eff/T=0.98–1.01，驱动层已支持），或 (b) 给球坐标步进补漂移项（侵入较大）。反讽注：正文第504行警告"手动归一化会改变热噪声统计权重、致仿真温度偏离设定值"——发布代码里真正把温度砍半的，是它默认保留给 fig2.11 的球坐标 stepper 本身。
...
### B — Boltzmann 有效温度 ✅（全量定案）
脚本 `scripts/11_boltzmann_teff/teff_dtsweep.py`（+ `merge_full_results.py` 合并、`euler_drift_discrimination.py` 机制判别）。零驱动纯单轴势阱（h_ex 置零，Δ=48.515 解析）；T_eff 用**精确有界基矩反演 + WLS 直方图斜率**（谐估计器构造偏置 0.989 仅列表不使用）；固定物理采样时长；τ_int + 长度平台稳态证明；dt→0 Richardson + block-bootstrap。
**最终数字（60ns×48轨×n_boot=2000；~90τ_int 充分平衡。cayley 段来自被系统重启杀死的首跑日志转录——种子化精确可复现；euler/midpoint 段来自 full_rest2；产物 `teff_full_merged.{json,png}`）**：

| 积分器 | T_eff/T @ dt=0.05/0.1/0.2/0.4 ps | dt→0 截距（矩估计） |
|---|---|---|
| Cayley 显式-ω（发布） | 0.978 / 0.987 / 1.003 / 1.007 | 0.9786±0.0144（-1.5σ） |
| Euler-球坐标 | 0.488 / 0.501 / 0.505 / 0.507 | **0.4920±0.0101（-50σ）** |
| Cayley 真中点（对照） | 0.979 / 1.027 / 0.960 / 1.024 | 0.9788±0.0136（-1.6σ） |

**Euler T/2 机制定案（三角证据链）**：(1) `switching()` θ̇/φ̇ 投影逐项对照标准球坐标 LL 展开**完全正确**，注入幅值与 cayley 共用（cayley 读 ≈1）→ 排除幅值/投影错误；(2) 全量斜率估计器读 0.56 而非 0.50——吻合"无 Jacobian 分布多出 −½ln u 项"的预言、不符纯幅值因子（两估计器应同读 0.50）；(3) **判别实验**：给 θ̇ 补上无自由参数的 Wong-Zakai 漂移 D·cotθ（D=½γ²_red(1+α²)σ_H²dt），T_eff/T 从 0.46 回到 **0.94/0.95（两估计器）**，达探针自身系统地板。诚实注记：深阱极限下仅凭矩估计无法区分漂移缺失与幅值减半（简并）；amp×√2 反例变体在探针规模下斜率读 1.02，单项鉴别力不足，判别依赖 (1)+(2)+(3) 闭合。
**Pilot 教训存档**：pilot 截距 0.829（欠平衡伪影）、规划探针 1.8–2.0×（估计器偏置伪影）、「单调过热趋势」（种子涨落）——三级对抗验证各挡下一个错误结论。
...
**结果**：见上表 C4/C13 行（判定"claim_false_confirmed"高置信）。系综复核确认 0.066 为运行实况。**⚠ 2026-07-05 更正**：F 系综跑的是**自热关闭**曲线（I_50≈1290–1308 µA，+13%），据此曾误判「12% 偏差、1% 匹配陈旧」；亲手复核自热开关两态后确认——**写入工况（自热开启）I_50≈1140 µA，与实验 1152 µA 吻合至约 1%**（Cayley/Euler 一致，ΔK_i/K_i≈−7% 的自热软化把阈值从 ~1300 拉到 ~1140）。故 §2.2.4.6/article 的「~1% 匹配」**正确**，θ_SH=0.066 标定无需改动，#12 实为已解决。教训：SER 阈值必须报明自热开/关态。高过驱动 P_sw 平台 ~0.78 可作图 2.11 讨论素材。
...
3. **SC5（Boltzmann 无修正）**：cayley 段已定——发布 Cayley 稳态 1–2% 内即 Boltzmann（截距 -1.5σ/-0.7σ），SC5 对发布格式**成立**，可保留但把依据改为"本工作 60ns 平衡实测"；**新增最高优先级条目**：Euler-球坐标 T_eff/T=0.50（缺 Wong-Zakai 漂移，机制+解析+实测三方吻合）→ fig2.11 须用 cayley 重跑或加显著性说明，且此缺陷应写入 §2.2.4.6 已修复/已识别缺陷清单（作为"RHS 一致性测试抓不到的系综级缺陷"教学点）。
...
7b. ~~**§2.2.4.6「匹配至 1% 以内」**：当前系综 I_50=1290 µA vs 1152 µA（12%）…~~ **【2026-07-05 撤销】** 上述基于自热关闭曲线，系误判。写入工况（自热开启）I_50≈1140 µA 与实验 1152 µA 吻合至约 1%——「1% 匹配」**成立**，θ_SH=0.066 无需重标定，#12 已解决。正文校准叙事保持 0.066 / ~1%（写入工况）即可。
