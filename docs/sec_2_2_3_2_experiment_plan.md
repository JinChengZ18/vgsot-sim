...
| C2 | SC5 Boltzmann 无需修正 | 5 | 被实现证伪(探针) | B | 🔄 **pilot 不能定论**——探针的 1.8–2.0× 未在无偏估计器下存活；dt→0 截距 0.829±0.133（-1.3σ）；60 ns 全量运行中 |
...
| C6 | SC3 强收敛阶 1.0 | 3 | depends(探针~0.5) | E | 🔄 运行中（GBM 门控 + 独立中点参考） |
| C7 | SC4 弱收敛阶 2.0 | 3 | depends | E | 🔄 运行中（双参考交叉验证） |
...
| C13 | θ_SH=0.04 还是 0.066 | 2 | 被实现证伪 | F | ✅ **证实**：运行时 `PhysicalConstantsConfig().theta_SH==0.066`，出图脚本无覆盖 → 图2.10标题/第743/76-77行溯源陈旧；200 种子系综 V_50 复核 ⏸ 运行中 |
...
### E — 随机强/弱收敛阶 🔄（重启运行中）
脚本 `scripts/11_strong_order/strong_order_brownian.py`（标量，GATE-0 GBM 门控 + GATE-1 确定性锚 + GATE-2 噪声主导比≥50 + 独立中点参考 + 自参考退化对照）、`scripts/10_weak_order/weak_order_audit.py`（按轨迹向量化，M=150k，双参考交叉验证，预平衡短horizon）。
首轮工作流中执行智能体因长段无输出被停滞判死（6 次）；脚本本身完整，现直接后台运行。**待办**：弱阶脚本的向量化 omega/cayley 是重实现——完成后须做标量-向量数值一致性抽查。
产物（预期）：`result/sec_2_2_3_2/E/strong_order.{png,json}`、`weak_order.{png,json}`。

### F — 图表与参数溯源审计 ✅（系综复核运行中）
脚本 `scripts/09_simulation_figures/integrator_audit.py`、`audit_theta_sh.py` + `tests/test_figure_provenance.py`。
**结果**：见上表 C4/C13 行（判定"claim_false_confirmed"高置信）。200 种子 NEGATIVE-I_SOT V_50 系综（θ_SH∈{0.066,0.04}×{cayley,euler}）后台运行中（~27min）。
产物：`result/sec_2_2_3_2/F/`。
