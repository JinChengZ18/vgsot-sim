...
Last updated: 2026-07-02 (RC deepening round: RTN node rigor fixes, `rtn/bridge.py` sLLG validation, `rtn/reservoir.py` reservoir layer + benchmarks, thesis §2.4 figures 2.20–2.22 / tables 2.14–2.15 via the `scripts/build_ppt_figs.py` panel flow — see `docs/maintenance/version_notes.md` 2026-07 and `scripts/10_rtn_reservoir/README.md`. Earlier 2026-05 round: opt-in toggles `R_series`, end-to-end `rng=…` plumbing, Psw↔SER display toggle, `rng_mode`/`integrator` selection; regression in `tests/test_toggles.py`.)
...
1. **`initialize.py` PAP name vs `tmr.py` resistance convention — RESOLVED 2026-07-21.** There was never a physics conflict: `tmr.py` defines $m_z = +1$ as parallel ($R_P \approx 5$ kΩ), and `initialize.py` sets `θ ≈ π` for `PAP=1`, so a `PAP=1` run genuinely *starts* antiparallel at $R_{AP} \approx 10$ kΩ and, with `target_mz=+1`, ends parallel — an AP→P event. Only the `PAP` name (and a docstring that called it "Parallel") read backwards; both are now corrected, and the audit is recorded in `CLAIMS.md`. The same audit found that the *experimental* direction labels had been inverted by `fit_from_loops.py`, which is fixed too; no numerical result changed.
