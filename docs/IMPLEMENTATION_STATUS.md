...
| 2.3.4 (calibration) | **θ_SH calibrated to V_th experiment** | `configs.PhysicalConstantsConfig.theta_SH=0.04` (was 0.25); scan in `09_simulation_figures/calibrate_to_experiment.py` | Re-calibration against Device A P→AP detailed-`P_sw` `V_th(0.75 ns) ≈ 894 mV`: `V_th^sim(0.75 ns) ≈ 903 mV` (~1% match), jointly with `TMR=1.0` and `RA=16.6e-12 Ω·m²`. `0.25` (literature β-W) still accepted as override for bare-material studies. |
