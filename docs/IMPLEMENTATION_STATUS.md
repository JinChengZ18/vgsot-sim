...
| 2.3.6 (RC) | **Random-telegraph-noise (RTN) reservoir primitive** — continuous-time 2-state Markov hopping with closed-form ⟨s⟩=tanh(ΔV/Vc0) and τ(V)=1/(r↑+r↓); exact propagator; `from_nb_fit` factory (ns units) | [`rtn/telegraph.py`](../src/vgsot_sim/rtn/telegraph.py) | New device primitive for reservoir computing; `tests/test_rtn_telegraph.py` (7). Complements (does not replace) the pulse-switching LLG. |
