...
- **Project reorganisation** (2026-05-16 → 2026-05-17 all-English paths): chapter-2 driver scripts live under [`scripts/`](scripts/) (`02_pdk`, `04_thermal_nonidealities`, `05_experimental_raw_data`, `06_psw_t_fitting`, `07_process_variability`, `08_sampling_effect`, `09_simulation_figures`), chapter prose + figures under [`article/00_chapter_drafts/`](article/00_chapter_drafts/). The originals one directory level up are retained as untouched read-only references; future updates target the in-repo copies.
